...
- Features:
  ...
  - Dockerfile + docker-compose.yml for `<internal-host>` deployment
...
| Anchor `.so` deployment | No localnet running on `<internal-host>` | Start `solana-test-validator`, deploy with `anchor deploy` |
| Algorand sandbox deploy | algod unreachable from `<internal-host>`; sandbox status unknown | Deploy via primary algod node or local `algokit localnet start` |
...
| `<internal-host>` deployment | Operator approval required per CLAUDE.md §17 | Copy relayer Docker image, create `.env`, `docker compose up` |
| `<internal-host>` dashboard deploy | Operator approval required | Add page to existing Next.js app, rebuild, restart nginx |
...
6. **Deploy infra**: Copy relayer and dashboard page to their respective internal hosts.
